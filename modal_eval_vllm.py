#!/usr/bin/env python3
"""Evaluate AWQ-quantized CyberSecQwen-4B on CTI-Bench using vLLM on Modal L4.
...
app = modal.App("cybersecqwen-eval-vllm")
...
class VllmEval:
    ...
    def start_server(self):
        print(f"Loading AWQ model: {MODEL_AWQ}")
    ...
    def eval_all(self, tasks: list[str]) -> dict[str, Any]:
        ...
        header = {
            "model": MODEL_AWQ,
            "quantization": "AWQ 4-bit (group_size=128, zero_point=True)",
            "gpu": GPU_TYPE,
            "protocol": {
                "temperature": TEMPERATURE,
                "max_tokens": MAX_TOKENS,
...
def main(task: str = "all"):
    ...
    print("=" * 60)
    print("  CyberSecQwen-4B-AWQ CTI-Bench Evaluation (vLLM)")
    print("=" * 60)
